<template>
  <div id="app" style="height: 500px;">
    <div id="SA" class="container main-container">
      <!-- <q-img src="../../assets/PS_A02.png" style="width: 80px;" /> -->
      <h1>Login</h1>
      <div class="container">
        <div class="container pb-0">
          ...
          <div>
            <input @keyup.enter="gotoLogin" v-model="pw" type="password" placeholder="Enter Password" class="form-control" name="pw" required>
          </div>
          <div>
            <button class="btnlgn btn-lg mb-0 text-white" @click="gotoLogin">Login</button>
          </div>
          <!-- <div class="mt-1">
           ...
           <hr class="line1 my-2">
          <div>
            <p class="mb-0">New to P.S.? Create an account.</p>
            <button class="btn btn-signup btn-lg text-white" @click="SignupModal">Signup</button>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    gotoLogin() {
      if (this.userId == '') {
        ...
        console.log("wow")
        this.Login({userId: this.userId, pw: this.pw})
        this.$emit('close')
      }
    },
...
.btnlgn {
  background: linear-gradient( to right, #503fff, #2ec4f9 );
  color: white;
}
